...
def running_one_epoch_inbreast(epoch, output_path, model, train_loader, val_loader, criterion, optimizer, device, threshold, beta, train_loss, train_acc, \
    valid_loss, valid_acc, writer, cam_method):
    model.train()
    epoch_loss = 0
    prediction = np.empty(shape=[0, 2], dtype=np.int)
    ...
    train_loss.append(epoch_loss)
    train_acc.append(acc)
    val_loss, val_acc, val_roc_auc, val_TP, val_FN, val_TN, val_FP = val_epoch_inbreast(epoch, output_path, model, val_loader, criterion, device, threshold, \
        beta, valid_loss, valid_acc, cam_method)
    val_sen = val_TP/(val_TP+val_FN)
    val_spe = val_TN/(val_TN+val_FP)
    writer.writerow({'epoch':epoch+1, 'train_loss':epoch_loss, 'train_acc':acc, 'train_auc':roc_auc, 'train_sen':sen, 'train_spe':spe, 'val_loss':val_loss,\
...
@torch.no_grad()
def val_epoch_inbreast(epoch, output_path, model, loader, criterion, device, threshold, beta, valid_loss, valid_acc, cam_method):
    global val_best_roc_auc
    model.eval()
    epoch_loss = 0
    prediction = np.empty(shape=[0, 2], dtype=np.int)
