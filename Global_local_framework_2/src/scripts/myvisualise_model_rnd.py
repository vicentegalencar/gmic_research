...
def visualize_saliency_patch_maps(model, output_path, loader, device, mask_dir):
    ...
    for step, (imgs, labels, test_filename) in enumerate(loader):
        ...
        layercam_extractor = LayerCAM(model, target_layer=['ds_net.layer4.1','ds_net.layer3.1']) #
        ...
        # import pdb; pdb.set_trace()
        #cam_extractor = CAM(model, fc_layer='left_postprocess_net.gn_conv_last',target_layer='ds_net.layer4.1.bn2')
        cam_extractor = CAM(model, fc_layer='left_postprocess_net.gn_conv_last',target_layer='ds_net.layer4.1')
