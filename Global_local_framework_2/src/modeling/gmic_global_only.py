...
class GMIC(nn.Module):
    def __init__(self, parameters):
        ...
        # # fusion branch
        if parameters["use_v1_global"]:
            self.fusion_dnn = nn.Linear(1024, 2)
        else:
    ...
    def forward(self, x_original):
        ...
        # self.y_fusion = torch.softmax(global_vec) dim=1)
        self.y_fusion = self.y_global

        #return self.y_global, self.y_local, self.y_fusion, self.saliency_map, y_score
        return self.y_global, self.y_fusion, self.saliency_map, y_score