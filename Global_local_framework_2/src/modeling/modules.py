...
class DownsampleNetworkResNet18V1(ResNetV1):
    ...
    def forward(self, x):
        ...
        # last_feature_map = self.mynet.forward(x)
...
class PostProcessingStandard(nn.Module):
    ...
    def __init__(self, parameters):
        ...
        if parameters["use_v1_global"] == False:
...
class GlobalNetwork(AbstractMILUnit):
    ...
    def __init__(self, parameters, parent_module):
        ...
        if "use_v1_global" in parameters and parameters["use_v1_global"]:
            ...
            # self.downsampling_branch = ResNetV1(64, BasicBlockV1, [2,2,2,2], 1)
            

            if "pretrain_global" in parameters and parameters["pretrain_global"]:
                ...
                self.downsampling_branch.load_state_dict(state_dict)
                # self.downsampling_branch.mynet.load_state_dict(state_dict)
        else:
    ...
    def forward(self, x):
        ...
        cam, cam_score = self.postprocess_module.forward(last_feature_map)
        return last_feature_map, cam, cam_score
...
class LocalNetwork(AbstractMILUnit):
    ...
    def add_layers(self):
        ...
        if "pretrain_local" in self.parameters and self.parameters["pretrain_local"]:
            import torch.hub
            state_dict = torch.hub.load_state_dict_from_url("https://download.pytorch.org/models/resnet18-5c106cde.pth",
                                                progress=True)
            del state_dict['fc.weight']
            del state_dict['fc.bias']
            # self.parent_module.dn_resnet = self.parent_module.dn_resnet.load_state_dict(state_dict)
            self.parent_module.dn_resnet.load_state_dict(state_dict)
